<template>
  <v-toolbar :color="mainColor" class="elevation-1" dark height="64">
    ...
    <v-toolbar-title>
      <v-list-item two-line class="pl-2 pr-0" @click="$router.push('profile')">
        <v-list-item-content>
          <v-list-item-title class="title mb-1">{{ title }}</v-list-item-title>
          <v-list-item-subtitle class="caption white--text">{{ time }}</v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>
    </v-toolbar-title>
    <v-spacer/>
...
<script lang="ts">
  ...
  import {mainColor} from '@/models/Constants'
	...
	export default class ChatTopBar extends Vue {
	  mainColor = mainColor
